...
**评判说明**：
1. **优先检查模板化内容**：在评估质量之前，必须先检查内容是否为模板化提示。如果内容为结构化提示而非实际记录，必须直接判为"不合格"，无需进行其他质量评估。
...
**输出要求**：
1. **优先检查**：首先检查是否为模板化内容，如果是，必须直接判为"不合格"，原因明确说明"模板化提示内容"。
2. 如内容不合格，指出所有主要问题点，并提出具体、可执行的改进建议；
3. 如内容合格，给出优化建议（即使达标也要指出可提升之处）；
4. 如内容优秀，指出1-2个突出亮点，说明其在计划性、目标性或推进力方面的优势；
5. 严格根据原始文本进行评判，不要虚构内容；
6. **严格只输出 JSON 格式，不要添加任何多余说明、解释或建议；**
7. **所有评估内容都必须包含在JSON结构内，不要在JSON外部添加任何文字；**
8. 如果内容为空，评估为"不合格"。
