...
【明确时间的定义（常规推进必查，合格从宽）】
- 合格门槛：正文可逐字引用到**可定位的时间范围**即视为「有明确时间」，**不要求**精确到具体日期或星期几。
- **合格即认可**（任一类即可）：相对周期（今日/明天/本周/下周/本月底/下月初/月底前；today/tomorrow/this week/next week/end of month 等）；相对星期（本周五/下周三/by Friday/next Monday 等）；具体日期。
- **典型合格**：「下周发送方案」「next week send proposal」——仅出现「下周/next week」等周级表述即满足时间安排，**不得**以「未写到哪一天」判不合格。
- **无明确时间**（仅此情形不得判合格）：全文无任何时间词或时间范围；仅有「安排会议/尽快/后续/待定」或 schedule a meeting / ASAP / follow up 等而无何时、何周、何月完成。
...
步骤3) 常规推进判定（须同时满足下列各条才可判合格；任一条不满足则判不合格，不得以「动作很具体」放宽）：
   ...
   - 优秀（仅常规推进场景）：在合格基础上**同时**满足下列 a–d（缺一不可）；任一条仅“勉强沾边”则最高判合格：
     ...
     b. 至少2个可定位的时间点（可为具体日期/星期，或两个不同周/月范围，如「本周发资料、下周演示」；周/月级表述可计为时间点；若仅1个周级时间且其余条件勉强，最高判合格）；
...
【收敛规则】
- 证据锚定：每个判断都必须能在原文找到直接证据（可逐字引用），不能脑补或推断。模板提示中的示例词不算证据。
- 冲突裁决：仅适用于「优秀」与「合格」之间边界不清时，**统一判「合格」**（评优秀宁缺毋滥）。不适用于「合格」与「不合格」：凡**完全无**时间范围表述、缺具体动作或缺推进结果等硬条件，必须判「不合格」；**不得**因时间仅为周/月粒度（如仅写「下周」）而判不合格。
