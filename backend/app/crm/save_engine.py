...
- 评估对象是**客户相关跟进**（拜访、触达、催款、法务、交接、关单等均适用），不是「必须完成一次双向拜访对话」。
- 评估的是**信息含量与业务价值**，不是排版格式；编号/分节标题本身不影响等级。
- 下列定义中的例句仅为代表，同类事实按同一原则计入，**不必与例句字面一致，也不得因未出现在列举中而否决**。

【有效沟通动作的定义（计入 actions）】
- **原则**：原文能定位到「做了什么 + 具体对象/渠道/事项/议题」，即为有效动作。不要求必须是产品讲解或当面沟通。
- **算有效**（任一类即可；逐字摘录或连续子串）：
  - 有议题落点的商务沟通。代表：「确认慢查询集中在报表模块」「演示索引优化与灰度步骤」「介绍了 APTSell 红绿灯质检」。
  - 有可核验渠道或工具的触达/事务（不要求产品功能点）。代表：「三次致电客户」「发了律师函」「联系了两家催款公司」。
- **拜访/见面/沟通**：必须落到议题或事项；**仅写出见了谁/哪家单位，不算有效动作**。
  「上门未见到 / 前台不转接」因含可核验的触达结果，可按触达动作计；单纯「拜访了某领导」不可。
- **「介绍/沟通/讨论/演示」类**（只约束商务沟通，不用于否决致电/发函等触达动作）：
  - 同段出现具体能力、场景、范围、问题或方案要点 → 算；
  - 仅笼统对象（产品/方案/合作/poc）且全文无落点 → 不算。
- **抽取提示**：具体落点可与笼统介绍同段出现；能摘到带落点的连续子串即可计入，
  **不得**因前文有「介绍产品/整体能力」而整段否决。
- **不算有效**：
  - 仅活动词无对象：已沟通/已讨论/已跟进/保持联系/had a call（对比：「三次致电客户」有渠道与对象 → 算）；
  - 仅拜访叙事：拜访了某领导/某单位/visited the client（无议题、也无未接通等触达结果）；
  - 仅笼统主题无落点：讨论方案/介绍产品/introduced the solution（且也不是带渠道/工具的触达动作）；
  - 纯过程套话：整体顺利/后续持续跟进/will follow up；
  - 占位：「（如：…）」「待补充」「TBD」「test」及同义；
  - 同一动作同义重复（只计 1 条）。

【客户反馈的定义（计入 feedbacks）】
- **原则**：原文能定位到「客户侧发生了什么」即为反馈。不限于口头观点；客户本人或相关第三方转述均可。
- **算具体反馈**（任一类即可；逐字摘录或连续子串）：
  a. 可摘录的观点、要求、确认、异议、顾虑、条件。
     代表：「担心迁移风险」「要求周五前给回滚预案」「认可红绿灯质检」。
  b. 可定位的客户侧结果或状态（本次未形成对话时尤其常见）：对方是否回应、能否联系上、对接是否仍有效、本次为何没谈成、客户立场或经营结论等。
     代表：「客户拒绝回应」「电话空号」「联系人已离职」「上门客户不在」「客户明确今年无预算」。
     同类客户侧事实一律计入，**不得**因「不是口头观点」而视为无反馈。
- **不算具体反馈**：
  - 笼统态度：认可/满意/有兴趣/OK，且未落到可定位的具体事项（某能力、功能、场景、问题、条件）；
    「对 poc/方案/产品/结果表示满意」仍属笼统，不算；「认可红绿灯质检」因事项具体，算。
  - 销售单方空泛转述：「客户整体认可方案」；
  - 无客户事实的套话：整体顺利/暂无异议；
  - 仅销售侧等待：等待客户反馈/后续等回复（未写出已触达后的客户侧结果）。
...
步骤2) 证据提取（定级前必须完成；所有条目须为原文逐字摘录或连续子串，不得改写）：
  - actions：列出全部有效沟通动作；
  - feedbacks：列出全部具体客户反馈（含客户侧结果/状态）。
    正文已有可定位的客户侧结果/状态时，**必须**写入 feedbacks，不得留空。
  分节标题、序号、「跟进内容/客户反馈/结论」等标签本身不得写入 actions 或 feedbacks。

步骤3) 按提取结果定级（仅依据 actions/feedbacks，不得脑补未出现的信息）：
  - **不合格**（任一即成立）：
    · len(actions) = 0；或
    · len(actions) ≥ 1 但 len(feedbacks) = 0；或
    · 仅有空泛套话（如「已沟通、客户认可、后续跟进」），提取后不足以达标。
    **禁止**：仅因「没有口头观点」判不合格——客户侧结果/状态须先计入 feedbacks，再定级。
  - **合格**（须同时满足）：
    · len(actions) ≥ 1；
    · len(feedbacks) ≥ 1；
    · 不要求口头观点或异议；观点/要求/顾虑/客户侧状态均可。
  - **优秀**（须同时满足；任一条仅勉强沾边则最高「合格」）：
    · len(actions) ≥ 2，且两条可区分（不同议题/动作，不得同义重复）；
    · len(feedbacks) ≥ 1，且至少 1 条为具体观点/要求/顾虑（**仅客户侧状态不足以支撑优秀**）；
    · 正文含可定位的业务细节，能体现推进价值或客户洞察。

【收敛规则】
- 先提取 evidence 再定 level；level 必须与 actions/feedbacks 一致，不得矛盾。
- **合格 vs 不合格**：len(actions) ≥ 1 且 len(feedbacks) ≥ 1 → 「合格」；抽不到有效 action → 「不合格」。
  有具体动作 + 可定位的客户侧信息（观点或状态）→ 倾向「合格」；
  **不得**以「没有口头反馈、仅有客户侧状态」判不合格；
  仅有叙事 + 笼统态度（顺利/认可/有兴趣/持续跟进）且无具体落点 → 「不合格」；
  仅有拜访对象 + 对 poc/方案/产品的笼统满意 → 「不合格」（缺有效动作或具体反馈）。
- **合格 vs 优秀**边界不清、或刚达下限且信息密度一般时，统一「合格」；反馈仅为客户侧状态时，最高「合格」。
- reason/reason_en 须点明关键证据或缺失项，不得与 level 矛盾。
- 不得因客户未开口/未接通/仅有客户侧状态而判不合格；不得因仅有编号格式而判不合格或降低等级。

【输出格式（仅 JSON）】
{{
  "followup_quality_zh": {{
    "actions": ["从原文摘录的有效沟通动作，无则为 []"],
    "feedbacks": ["从原文摘录的具体客户反馈（含客户侧结果/状态），无则为 []"],
    "level": "不合格|合格|优秀",
...
- 正文已有可定位的客户侧结果/状态时，feedbacks 不得为空，且不得因此判「不合格」。
...
【有效动作的定义（计入 actions）】
- **算有效**：正文可定位到可独立执行的具体动作（发送/提交/安排/演示/评审/联调/报价/电话沟通等 + 明确对象或事项）。
  例：「发送 PoC 方案」「安排技术评审」「提交 PoC 清单」「完成试点环境联调」
      「发送报价」「电话沟通采购条款」「给刘总发两家落地案例」。
- **「安排会议/沟通/对齐/同步」类动词判定**：
  - 同句或同段点明目的、议题、对象事项（如展示案例、确认范围、沟通采购条款）→ 算有效；
  - 仅有活动词、无任何事项落点 → 不算。
- **抽取提示**：具体事项可与笼统活动词同段出现；只要能摘到带明确对象/事项的连续子串，即可计入 actions，
  **不得**因出现「开会/沟通/对齐」字样而整段否决。
- **不算有效**（不得计入 actions）：
  - 空泛维持：保持沟通/等待反馈/持续跟进/stay in touch/waiting for feedback/后续跟进 等；
  - 仅活动无事项：安排会议/进一步沟通/对齐/同步 等，且全文未说明目的与内容；
  - 占位或提示：「（如：…）」「待补充」「TBD」「N/A」「test」「asdf」及同义占位；
  - 同一动作同义重复（只计 1 条）。

【推进目标/结果的定义（计入 outcomes；合格从宽）】
- **算有效**：具体且完成后可判断是否达成的目标结果；**也可由具体动作合理隐含**（不必单独写出「目标是…」句式）。
  例（显式）：「确认一期范围/测试范围」「推动客户内部立项」「锁定一期范围」「签署合同」。
  例（由动作隐含，可计入 outcomes）：
      「发送报价」→ 完成报价送达/供客户评估；
      「电话沟通采购条款」→ 推进采购条款对齐；
      「发送落地案例 / 演示红绿灯质检」→ 供客户评估合作或确认能力。
- **抽取提示**：若 actions 已含可验证事项，可将该事项的达成状态写入 outcomes（尽量用原文子串；若需概括，须紧扣原文动作，不得脑补未出现的商务结果如「签约」）。
- **不算有效**（不得计入 outcomes）：
  - 纯活动性空壳：「沟通」「讨论」「对齐」「同步」「跟进」等，无任何可验证事项；
  - 空泛维持类目标：「保持联系」「等待反馈」。
...
步骤3) 证据提取（closed_case 为 false 时定级前必须完成）：
  - actions：全部有效动作（原文逐字摘录或连续子串，不得改写）；
  - time_points：全部有效时间点（按「明确时间」定义；原文子串）；
  - outcomes：全部有效推进目标/结果（显式原文子串优先；若由具体动作隐含，可写入紧扣该动作的可验证事项，不得脑补原文未出现的商务结果）。
  分节标题、序号、「待办事项/时间节点/预期成果」等标签本身不得写入上述数组。
  动作与时间点可同句出现，分别计入对应数组（如「下周发送方案」→ action + time_point + 隐含 outcome 各可计 1 条）。

步骤4) 按提取结果定级（closed_case 为 true 时跳过；仅依据 actions/time_points/outcomes，不得脑补未出现的信息）：
  - **不合格**（任一即成立）：
    · len(actions) = 0；或
    · len(time_points) = 0；或
    · len(outcomes) = 0（且 actions 也无法隐含任何可验证事项）；或
    · 仅有空泛维持类表述（如「保持沟通，等待客户反馈」「下周持续跟进客户」——有时间但无具体可执行动作或无有效 outcome）。
  - **合格**（须同时满足）：
    · len(actions) ≥ 1；
    · len(time_points) ≥ 1；
    · len(outcomes) ≥ 1（含由具体动作合理隐含的可验证事项）。
  - **优秀**（须同时满足；任一条仅勉强沾边则最高判「合格」）：
    · len(actions) ≥ 2，且两条可区分（不同动作，不得同义重复）；
    · len(time_points) ≥ 2（可为两个不同日期/星期，或两个不同周/月范围；若仅 1 个周级时间且其余条件勉强，最高判合格）；
    · len(outcomes) ≥ 1，且至少 1 条为**显式**可验证推进结果（非仅由动作隐含、非活动性描述）；
    · 计划整体体现前瞻性与主动性（动作有递进关系或明确推进节奏）。

【收敛规则】
- 先提取 evidence 再定 level；level 必须与 actions/time_points/outcomes 及 closed_case 一致，不得矛盾。
- **合格 vs 不合格**：缺动作或缺时间必须判「不合格」；有具体动作+时间、推进目标仅由动作隐含 → 倾向「合格」；
  **不得**因时间仅为周/月粒度（如仅写「下周」）而判不合格；
  **不得**仅因未写「目标是…」句式、而否定可由动作隐含的 outcome。
- 全文仅有「下周持续跟进/保持沟通/等待反馈」等空泛维持 → 「不合格」。
- **合格 vs 优秀**边界不清、或刚达下限且计划覆盖范围有限时，统一判「合格」；仅靠动作隐含 outcome、无显式推进目标时，最高「合格」。
...
- 若 reason 写明缺少时间安排/无明确时间/缺具体动作，level 必须为「不合格」（closed_case 为 true 时除外）。
- 有具体动作+时间时，不得仅以「缺推进目标/缺预期成果」判不合格（应先按动作隐含规则提取 outcomes）。
- 若目标仅为过程性「沟通/讨论/同步」且无事项落点，level 不得为「优秀」；若同时无有效动作，则「不合格」。
- reason_en 为地道英文，禁止仅写「与中文一致」类占位。
- 使用双引号；可被标准 JSON 解析。
"""
