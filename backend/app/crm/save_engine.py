...
- **算有效**：正文可定位到「做了什么，且涉及什么具体议题/对象/范围」的独立事实（可逐字引用或连续子串）。
  例：「确认慢查询集中在报表模块」「演示索引优化与灰度步骤」「说明读写分离改造范围」
      「介绍了 APTSell 红绿灯质检及拜访状态可视化」。
- **「介绍/沟通/讨论/演示」类动词判定**：
  - 后接或同段出现具体产品能力、功能点、场景、范围、问题、方案要点 → 算有效；
  - 仅有笼统对象（产品/方案/能力/合作/升级）且**全文**无更具体议题落点 → 不算。
- **抽取提示**：具体议题可与笼统介绍同段出现；只要能摘到带具体落点的连续子串，即可计入 actions，
  **不得**因前文有「介绍产品/整体能力」而整段否决。
- **不算有效**（不得计入 actions，也不得作为合格依据）：
  - 仅活动词无对象：已沟通/已讨论/已跟进/保持联系/discussed/had a call 等；
  - 仅笼统主题且无具体落点：讨论方案/介绍产品/沟通升级/introduced the solution 等（全文未出现功能点、场景、范围、问题等）；
...
- **算具体反馈**：客户/对方可逐字引用的观点、要求、确认、异议、顾虑、条件、优先级等。
  例：「担心历史数据迁移风险」「要求周五前给回滚预案」「确认先覆盖订单查询链路」「可先在测试库验证」
      「认可红绿灯质检」「更关注落地效果，希望展示落地案例」。
- **不算具体反馈**（不得计入 feedbacks）：
  - 笼统态度且无对象：认可/满意/有兴趣/OK/positive/interested 等，未说明认可什么、感兴趣什么或顾虑什么；
    （若点明具体对象，如「认可红绿灯质检」「有兴趣看落地案例」→ 算具体反馈）
...
- 全文同时出现「可定位业务议题」（功能点/场景/范围/问题等）+「客户具体诉求或兴趣点」→ 倾向「合格」；
  仅有拜访叙事 + 笼统态度（顺利/认可/有兴趣/持续跟进）且无具体落点 → 「不合格」。
