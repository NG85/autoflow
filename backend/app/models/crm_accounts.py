...
class CRMAccount(SQLModel, table=True):
    ...
    last_follow_up: Optional[datetime] = Field(sa_column=Column(DateTime(timezone=True), nullable=True), description="最后跟进时间")
    last_deal_time: Optional[datetime] = Field(sa_column=Column(DateTime(timezone=True), nullable=True), description="最后一次成交时间")
    allocation_time: Optional[datetime] = Field(sa_column=Column(DateTime(timezone=True), nullable=True), description="领取/分配时间")
    ...
    creation_time: Optional[datetime] = Field(sa_column=Column(DateTime(timezone=True), nullable=True), description="创建时间")
    last_modifier: Optional[str] = Field(nullable=True, max_length=255, description="最后修改人")
    last_modification_time: Optional[datetime] = Field(sa_column=Column(DateTime(timezone=True), nullable=True), description="最后修改时间")
    ...
    earliest_deal_date: Optional[datetime] = Field(sa_column=Column(DateTime(timezone=True), nullable=True), description="最早成交日期")
    latest_deal_date: Optional[datetime] = Field(sa_column=Column(DateTime(timezone=True), nullable=True), description="最新成交日期")
