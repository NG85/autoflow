...
def trigger_crm_writeback_task(
    ...
    writeback_mode: Optional[str] = Body(
        None,
        description="拜访回写模式：CBG / APAC / OLM / CHAITIN / WEBEYE / FENBEITONG / LIEPIN / WYWJ / ZHIPU 等；不传则用 CRM_WRITEBACK_DEFAULT_MODE。二者均为空时无法触发；未实现该模式的拜访回写时任务成功结束且 writeback_count=0。",
    ),
):
    """
    手动触发CRM拜访记录回写任务。
...
def trigger_crm_writeback_by_ids(
    ...
    writeback_mode: Optional[str] = Body(
        None,
        description="拜访回写模式：CBG / APAC / OLM / CHAITIN / WEBEYE / FENBEITONG / LIEPIN / WYWJ / ZHIPU 等；不传则用 CRM_WRITEBACK_DEFAULT_MODE。二者均为空时无法执行。",
    )
):
    """
    根据指定拜访记录ID进行CRM回写
