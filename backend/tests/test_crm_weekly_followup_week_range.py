...
def test_completed_week_on_saturday():
...
def test_completed_week_on_monday():
    # 2025-12-29 周一 -> 仍指上一完整周六~周五
    start, end = resolve_weekly_followup_week_range(date(2025, 12, 29), week_range_mode="completed")
    assert start == date(2025, 12, 20)
    assert end == date(2025, 12, 26)
