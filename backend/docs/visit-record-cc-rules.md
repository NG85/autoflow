...
抄送名单仅来自 `notification_cc_rules`：`scope_type=user` 按录入人差异化抄送，`scope_type=global` 表达公司级旁观（每次拜访都抄送）。
...
| 抄送 | `notification_cc_rules`（含 global）；使用 leader 版卡片模板 |
...
- 按部门、按商机等复杂匹配（表结构预留；当前实现 `user` + `global`）。
...
send_visit_record_notification
  └─ _collect_visit_record_recipients_and_groups
       ...
       ├─ resolve_visit_record_cc_recipients   # notification_cc_rules（user + global）
...
| `configured_cc` | leader 版 | 配置表解析出的抄送人（`cc_scope=user` 或 `global`） |
...
| `scope_type` | VARCHAR(32) | 匹配维度；`user` / `global`；预留 `department` |
...
- **按 `user_id` 去重**，得到 `configured_cc_user_ids`；同一人同时命中 user / global 时 `cc_scope` 取 `user`。
...
- 个人列表中的 `leader`、`cc_scope=global` 的 `configured_cc` 被移除；
...
```
路由：notification_cc_rules（scope_type=user | global）
...
| `app/repositories/notification_cc_rule.py` | 按 recorder 查 user + global 规则并合并 recipients |
